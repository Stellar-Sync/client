...
public sealed class IpcCallerGlamourer : DisposableMediatorSubscriberBase, IIpcCaller
{
    ...
    public IpcCallerGlamourer(ILogger<IpcCallerGlamourer> logger, IDalamudPluginInterface pi, DalamudUtilService dalamudUtil, StellarMediator stellarMediator,
        RedrawManager redrawManager) : base(logger, stellarMediator)
    {
    ...
    public void CheckAPI()
    {
        ...
        finally
        {
            if (!apiAvailable && !_shownGlamourerUnavailable)
            {
                ...
                _stellarMediator.Publish(new NotificationMessage("Glamourer inactive", "Your Glamourer installation is not active or out of date. Update Glamourer to continue to use Stellar. If you just updated Glamourer, ignore this message.",
        ...
        try
        {
            ...
            await _redrawManager.PenumbraRedrawInternalAsync(logger, handler, applicationId, (chara) =>
            {
                try
                {
                    ...
                    _stellarMediator.Publish(new PenumbraRedrawCharacterMessage(chara));
                }
                catch (Exception ex)
                {
    ...
    {
        _stellarMediator.Publish(new GlamourerChangedMessage(address));
    }
}
