...
public class IpcProvider : IHostedService, IMediatorSubscriber
{
    ...
    public StellarMediator Mediator { get; init; }

    public IpcProvider(ILogger<IpcProvider> logger, IDalamudPluginInterface pi,
        CharaDataManager charaDataManager, StellarMediator stellarMediator)
    {
        _logger = logger;
        _pi = pi;
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        _loadFileProvider = _pi.GetIpcProvider<string, IGameObject, bool>("StellarSync.LoadMcdf");
        _loadFileProvider.RegisterFunc(LoadMcdf);
        _loadFileAsyncProvider = _pi.GetIpcProvider<string, IGameObject, Task<bool>>("StellarSync.LoadMcdfAsync");
        _loadFileAsyncProvider.RegisterFunc(LoadMcdfAsync);
        _handledGameAddresses = _pi.GetIpcProvider<List<nint>>("StellarSync.GetHandledAddresses");
