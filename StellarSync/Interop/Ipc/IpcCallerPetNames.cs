...
public sealed class IpcCallerPetNames : IIpcCaller
{
    ...
    {
        CheckAPI();
        _stellarMediator.Publish(new PetNamesReadyMessage());
    }

    private void OnPetNicknamesDispose()
    {
        _stellarMediator.Publish(new PetNamesMessage(string.Empty));
    }
    ...
    {
        _stellarMediator.Publish(new PetNamesMessage(data));
    }
