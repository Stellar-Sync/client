...
public class FileTransferOrchestrator : DisposableMediatorSubscriberBase
{
    ...
    private readonly StellarConfigService _stellarConfig;
    ...
    public FileTransferOrchestrator(ILogger<FileTransferOrchestrator> logger, StellarConfigService stellarConfig,
        StellarMediator mediator, TokenProvider tokenProvider, HttpClient httpClient) : base(logger, mediator)
    {
        _stellarConfig = stellarConfig;
        _tokenProvider = tokenProvider;
        _httpClient = httpClient;
        var ver = Assembly.GetExecutingAssembly().GetName().Version;
        _httpClient.DefaultRequestHeaders.UserAgent.Add(new ProductInfoHeaderValue("StellarSync", ver!.Major + "." + ver!.Minor + "." + ver!.Build));

        _availableDownloadSlots = stellarConfig.Current.ParallelDownloads;
    ...
    public async Task WaitForDownloadSlotAsync(CancellationToken token)
    {
        ...
        {
            if (_availableDownloadSlots != _stellarConfig.Current.ParallelDownloads && _availableDownloadSlots == _downloadSemaphore.CurrentCount)
            {
                _availableDownloadSlots = _stellarConfig.Current.ParallelDownloads;
                _downloadSemaphore = new(_availableDownloadSlots, _availableDownloadSlots);
            }
        }
    ...
    {
        var limit = _stellarConfig.Current.DownloadSpeedLimitInBytes;
        if (limit <= 0) return 0;
        limit = _stellarConfig.Current.DownloadSpeedType switch
        {
            StellarConfiguration.Models.DownloadSpeeds.Bps => limit,
            StellarConfiguration.Models.DownloadSpeeds.KBps => limit * 1024,
            StellarConfiguration.Models.DownloadSpeeds.MBps => limit * 1024 * 1024,
            _ => limit,
        };
