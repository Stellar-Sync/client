...
public sealed partial class ApiController : DisposableMediatorSubscriberBase, IStellarHubClient
{
    public const string MainServer = "Stellar Sync Server";
    public const string MainServiceUri = "wss://stellar.kasu.network";
    ...
    private HubConnection? _stellarHub;
    private ServerState _serverState;
    private CensusUpdateMessage? _lastCensus;

    public ApiController(ILogger<ApiController> logger, HubFactory hubFactory, DalamudUtilService dalamudUtil,
        PairManager pairManager, ServerConfigurationManager serverManager, StellarMediator mediator,
        TokenProvider tokenProvider, StellarConfigService stellarConfigService) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<HubClosedMessage>(this, (msg) => StellarHubOnClosed(msg.Exception));
        Mediator.Subscribe<HubReconnectedMessage>(this, (msg) => _ = StellarHubOnReconnectedAsync());
        Mediator.Subscribe<HubReconnectingMessage>(this, (msg) => StellarHubOnReconnecting(msg.Exception));
    ...
    {
        return await _stellarHub!.InvokeAsync<bool>(nameof(CheckClientHealth)).ConfigureAwait(false);
    }
    ...
    public async Task CreateConnectionsAsync()
    {
        ...
        if (!_serverManager.CurrentServer.UseOAuth2)
        {
            ...
            if (multi)
            {
                ...
                _connectionDto = null;
                Mediator.Publish(new NotificationMessage("Multiple Identical Characters detected", "Your Service configuration has multiple characters with the same name and world set up. Delete the duplicates in the character management to be able to connect to Stellar.",
        ...
        while (ServerState is not ServerState.Connected && !token.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                catch (StellarAuthFailureException ex)
                {
                ...
                _stellarHub = _hubFactory.GetOrCreate(token);
                InitializeApiHooks();

                await _stellarHub.StartAsync(token).ConfigureAwait(false);
                ...
                if (_connectionDto.ServerVersion != IStellarHub.ApiVersion)
                {
                    if (_connectionDto.CurrentClientVersion > currentClientVer)
                    {
                        Mediator.Publish(new NotificationMessage("Client incompatible",
                            ...
                            $"This client version is incompatible and will not be able to connect. Please update your Stellar Sync client.",
                            NotificationType.Error));
                    }
                ...
                if (_connectionDto.CurrentClientVersion > currentClientVer)
                {
                    Mediator.Publish(new NotificationMessage("Client outdated",
                        $"Your client is outdated ({currentClientVer.Major}.{currentClientVer.Minor}.{currentClientVer.Build}), current is: " +
                        $"{_connectionDto.CurrentClientVersion.Major}.{_connectionDto.CurrentClientVersion.Minor}.{_connectionDto.CurrentClientVersion.Build}. " +
                        $"Please keep your Stellar Sync client up-to-date.",
                ...
                if (_dalamudUtil.HasModifiedGameFiles)
                {
                    ...
                    if (!_stellarConfigService.Current.DebugStopWhining)
                        Mediator.Publish(new NotificationMessage("Modified Game Files detected",
                            ...
                            "Stellar Sync, Penumbra, and some other plugins assume your FFXIV installation is unmodified in order to work. " +
                ...
                if (_dalamudUtil.IsLodEnabled && !_naggedAboutLod)
                {
                    ...
                    if (!_stellarConfigService.Current.DebugStopWhining)
                    {
                        Mediator.Publish(new NotificationMessage("Model LOD is enabled",
                            ...
                            "random crashes when loading in or rendering modded pairs. Disabling LOD has a very low performance impact. Disable LOD while using Stellar: " +
    ...
    {
        var dto = await _stellarHub!.InvokeAsync<ConnectionDto>(nameof(GetConnectionDto)).ConfigureAwait(false);
    ...
    {
        while (!ct.IsCancellationRequested && _stellarHub != null)
        {
    ...
    private void StellarHubOnClosed(Exception? arg)
    {
        _healthCheckTokenSource?.Cancel();
    ...
    private async Task StellarHubOnReconnectedAsync()
    {
        ServerState = ServerState.Reconnecting;
        try
        {
    ...
    private void StellarHubOnReconnecting(Exception? arg)
    {
