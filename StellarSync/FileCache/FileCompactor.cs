...
public sealed class FileCompactor
{
    ...
    public FileCompactor(ILogger<FileCompactor> logger, StellarConfigService stellarConfigService, DalamudUtilService dalamudUtilService)
    {
    ...
    public void CompactStorage(bool compress)
    {
        ...
        int currentFile = 1;
        var allFiles = Directory.EnumerateFiles(_stellarConfigService.Current.CacheFolder).ToList();
    ...
    public async Task WriteAllBytesAsync(string filePath, byte[] decompressedFile, CancellationToken token)
    {
        ...
        if (_dalamudUtilService.IsWine || !_stellarConfigService.Current.UseCompactor)
        {
            return;
        }
