...
public sealed class CacheMonitor : DisposableMediatorSubscriberBase
{
    ...
    public CacheMonitor(ILogger<CacheMonitor> logger, IpcManager ipcManager, StellarConfigService configService,
        FileCacheManager fileDbManager, StellarMediator mediator, PerformanceCollectorService performanceCollector, DalamudUtilService dalamudUtil,
        FileCompactor fileCompactor) : base(logger, mediator)
    {
        ...
        Mediator.Subscribe<PenumbraInitializedMessage>(this, (_) =>
        {
            ...
            StartStellarWatcher(configService.Current.CacheFolder);
            InvokeScan();
        });
    ...
    private readonly Dictionary<string, WatcherChange> _stellarChanges = new Dictionary<string, WatcherChange>(StringComparer.OrdinalIgnoreCase);

    public void StopMonitoring()
    {
        Logger.LogInformation("Stopping monitoring of Penumbra and Stellar storage folders");
        StellarWatcher?.Dispose();
    ...
    public void StartStellarWatcher(string? stellarPath)
    {
        StellarWatcher?.Dispose();
        if (string.IsNullOrEmpty(stellarPath) || !Directory.Exists(stellarPath))
        {
            StellarWatcher = null;
            Logger.LogWarning("Stellar file path is not set, cannot start the FSW for Stellar.");
            return;
        }
        ...
        Logger.LogInformation("Stellar Storage is on NTFS drive: {isNtfs}", StorageisNTFS);

        Logger.LogDebug("Initializing Stellar FSW on {path}", stellarPath);
        StellarWatcher = new()
        {
            Path = stellarPath,
        ...
        StellarWatcher.Deleted += StellarWatcher_FileChanged;
        StellarWatcher.Created += StellarWatcher_FileChanged;
        StellarWatcher.EnableRaisingEvents = true;
    }

    private void StellarWatcher_FileChanged(object sender, FileSystemEventArgs e)
    {
        Logger.LogTrace("Stellar FSW: FileChanged: {change} => {path}", e.ChangeType, e.FullPath);

        if (!AllowedFileExtensions.Any(ext => e.FullPath.EndsWith(ext, StringComparison.OrdinalIgnoreCase))) return;

        lock (_watcherChanges)
        {
            _stellarChanges[e.FullPath] = new(e.ChangeType);
        }

        _ = StellarWatcherExecution();
    }
    ...
    private CancellationTokenSource _stellarFswCts = new();
    public FileSystemWatcher? PenumbraWatcher { get; private set; }
    public FileSystemWatcher? StellarWatcher { get; private set; }

    private async Task StellarWatcherExecution()
    {
        _stellarFswCts = _stellarFswCts.CancelRecreate();
        var token = _stellarFswCts.Token;
        var delay = TimeSpan.FromSeconds(5);
        Dictionary<string, WatcherChange> changes;
        lock (_stellarChanges)
            changes = _stellarChanges.ToDictionary(t => t.Key, t => t.Value, StringComparer.Ordinal);
        try
        {
            do
            {
    ...
    private void FullFileScan(CancellationToken ct)
    {
        ...
        if (string.IsNullOrEmpty(_configService.Current.CacheFolder) || !Directory.Exists(_configService.Current.CacheFolder))
        {
            ...
            Logger.LogWarning("Stellar Cache directory is not set or does not exist.");
        }
        ...
        if (!_configService.Current.InitialScanComplete)
        {
            ...
            _configService.Save();
            StartStellarWatcher(_configService.Current.CacheFolder);
