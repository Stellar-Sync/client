...
public sealed class FileCacheManager : IHostedService
{
    ...
    public FileCacheManager(ILogger<FileCacheManager> logger, IpcManager ipcManager, StellarConfigService configService, StellarMediator stellarMediator)
    {
        _logger = logger;
    ...
    {
        _stellarMediator.Publish(new HaltScanMessage(nameof(ValidateLocalIntegrity)));
        ...
        _stellarMediator.Publish(new ResumeScanMessage(nameof(ValidateLocalIntegrity)));
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        if (File.Exists(_csvPath))
        {
            ...
            {
                _stellarMediator.Publish(new NotificationMessage("Penumbra not connected",
                    "Could not load local file cache data. Penumbra is not connected or not properly set up. Please enable and/or configure Penumbra properly to use Stellar. After, reload Stellar in the Plugin installer.",
                    StellarConfiguration.Models.NotificationType.Error));
            }
