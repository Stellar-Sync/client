...
public class SettingsUi : WindowMediatorSubscriberBase
{
    ...
    private readonly StellarConfigService _configService;
    ...
    public SettingsUi(ILogger<SettingsUi> logger,
        UiSharedService uiShared, StellarConfigService configService,
        PairManager pairManager,
        ServerConfigurationManager serverConfigurationManager,
        PlayerPerformanceConfigService playerPerformanceConfigService,
        StellarMediator mediator, PerformanceCollectorService performanceCollector,
    ...
    private void DrawBlockedTransfers()
    {
        ...
        UiSharedService.ColorTextWrapped("Files that you attempted to upload or download that were forbidden to be transferred by their creators will appear here. " +
                             ...
                             "Ask your paired friend to send you the mod in question through other means, acquire the mod yourself or pester the mod creator to allow it to be sent over Stellar.",
    ...
    private void DrawFileStorageSettings()
    {
        ...
        if (_uiShared.IconTextButton(FontAwesomeIcon.Running, "Open Stellar Character Data Hub"))
        {
        ...
        UiSharedService.TextWrapped("Stellar stores downloaded files from paired people permanently. This is to improve loading performance and requiring less downloads. " +
        ...
        ImGui.AlignTextToFramePadding();
        ImGui.TextUnformatted("Monitoring Stellar Storage Folder: " + (_cacheMonitor.StellarWatcher?.Path ?? "Not monitoring"));
        if (string.IsNullOrEmpty(_cacheMonitor.StellarWatcher?.Path))
        {
            ImGui.SameLine();
            using var id = ImRaii.PushId("stellarMonitor");
            if (_uiShared.IconTextButton(FontAwesomeIcon.ArrowsToCircle, "Try to reinitialize Monitor"))
            {
                _cacheMonitor.StartStellarWatcher(_configService.Current.CacheFolder);
            }
        }
        if (_cacheMonitor.StellarWatcher == null || _cacheMonitor.PenumbraWatcher == null)
        {
            ...
            UiSharedService.AttachToolTip("Attempts to resume monitoring for both Penumbra and Stellar Storage. "
        ...
        else
        {
            ...
            UiSharedService.AttachToolTip("Stops the monitoring for both Penumbra and Stellar Storage. "
                + "Do not stop the monitoring, unless you plan to move the Penumbra and Stellar Storage folders, to ensure correct functionality of Stellar." + Environment.NewLine
        ...
        {
            UiSharedService.ColorTextWrapped("Hint: To free up space when using Stellar consider enabling the File Compactor", ImGuiColors.DalamudYellow);
        }
        ...
        if (!_fileCompactor.MassCompactRunning)
        {
            ...
            UiSharedService.AttachToolTip("This will run compression on all files in your current Stellar Storage." + Environment.NewLine
            ...
            UiSharedService.AttachToolTip("This will run decompression on all files in your current Stellar Storage.");
        }
        else
        {
        ...
        ImGui.Separator();
        UiSharedService.TextWrapped("File Storage validation can make sure that all files in your local Stellar Storage are valid. " +
        ...
        UiSharedService.AttachToolTip("You normally do not need to do this. THIS IS NOT SOMETHING YOU SHOULD BE DOING TO TRY TO FIX SYNC ISSUES." + Environment.NewLine
            ...
            + "Stellars storage is self-clearing and will not surpass the limit you have set it to." + Environment.NewLine
    ...
    private void DrawGeneral()
    {
        ...
        _uiShared.DrawHelpText("This will add Stellar related right click menu entries in the game UI on paired players.");
        ...
        _uiShared.DrawHelpText("This will add Stellar connection status and visible pair count in the Server Info Bar.\nYou can further configure this through your Dalamud Settings.");
        ...
        if (ImGui.Checkbox("Show Stellar Profiles on Hover", ref showProfiles))
        {
    ...
    private void DrawPerformance()
    {
        ...
        _uiShared.DrawHelpText("Will show a performance indicator when players exceed defined thresholds in Stellars UI." + Environment.NewLine + "Will use warning thresholds.");
        ...
        _uiShared.DrawHelpText("Stellar will print a warning in chat once per session of meeting those people. Will not warn on players with preferred permissions.");
        using (ImRaii.Disabled(!warnOnExceedingThresholds && !showPerformanceIndicator))
        {
            ...
            _uiShared.DrawHelpText("Stellar will also print warnings and show performance indicator for players where you enabled preferred permissions. If warning in general is disabled, this will not produce any warnings.");
        }
    ...
    private void DrawServerConfiguration()
    {
        ...
        if (ImGui.BeginTabBar("serverTabBar"))
        {
            if (ImGui.BeginTabItem("Character Management"))
            {
                ...
                {
                    UiSharedService.ColorTextWrapped("Characters listed here will automatically connect to the selected Stellar service with the settings as provided below." +
                    ...
                    foreach (var item in selectedServer.Authentications.ToList())
                    {
                        ...
                        if (ImGui.Checkbox("Automatically login to Stellar", ref isAutoLogin))
                        {
                            item.AutoLogin = isAutoLogin;
                            _serverConfigurationManager.Save();
                        }
                        _uiShared.DrawHelpText("When enabled and logging into this character in XIV, Stellar will automatically connect to the current service.");
            ...
            if (ImGui.BeginTabItem("Service Configuration"))
            {
                ...
                if (_dalamudUtilService.IsWine)
                {
                    ...
                    _uiShared.DrawHelpText("On wine, Stellar will automatically fall back to ServerSentEvents/LongPolling, even if WebSockets is selected. "
    ...
    private async Task<(bool Success, bool partialSuccess, string Result)> ConvertSecretKeysToUIDs(ServerStorage serverStorage, CancellationToken token)
    {
        ...
        var oauthCheckUri = StellarAuth.GetUIDsBasedOnSecretKeyFullPath(new Uri(baseUri));
