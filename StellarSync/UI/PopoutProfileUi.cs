...
public class PopoutProfileUi : WindowMediatorSubscriberBase
{
    ...
    public PopoutProfileUi(ILogger<PopoutProfileUi> logger, StellarMediator mediator, UiSharedService uiBuilder,
        ServerConfigurationManager serverManager, StellarConfigService stellarConfigService,
        StellarProfileManager stellarProfileManager, PairManager pairManager, PerformanceCollectorService performanceCollectorService) : base(logger, mediator, "###StellarSyncPopoutProfileUI", performanceCollectorService)
    {
        ...
        Mediator.Subscribe<CompactUiChange>(this, (msg) =>
        {
            ...
            if (stellarConfigService.Current.ProfilePopoutRight)
            {
    ...
    protected override void DrawInternal()
    {
        ...
        try
        {
            ...
            var stellarProfile = _stellarProfileManager.GetStellarProfile(_pair.UserData);

            if (_textureWrap == null || !stellarProfile.ImageData.Value.SequenceEqual(_lastProfilePicture))
            {
                _textureWrap?.Dispose();
                _lastProfilePicture = stellarProfile.ImageData.Value;
                _textureWrap = _uiSharedService.LoadImage(_lastProfilePicture);
            }

            if (_supporterTextureWrap == null || !stellarProfile.SupporterImageData.Value.SequenceEqual(_lastSupporterPicture))
            {
                _supporterTextureWrap?.Dispose();
                _supporterTextureWrap = null;
                if (!string.IsNullOrEmpty(stellarProfile.Base64SupporterPicture))
                {
                    _lastSupporterPicture = stellarProfile.SupporterImageData.Value;
            ...
            var descText = stellarProfile.Description;
            ...
            UiSharedService.TextWrapped(trimmed ? descText + $"...{Environment.NewLine}[Open Full Profile for complete description]" : stellarProfile.Description);
            font.Dispose();
