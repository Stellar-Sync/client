...
namespace StellarSync.UI.Handlers;
...
public class IdDisplayHandler
{
    ...
    public IdDisplayHandler(StellarMediator mediator, ServerConfigurationManager serverManager, StellarConfigService stellarConfigService)
    {
        _mediator = mediator;
    ...
    public void DrawPairText(string id, Pair pair, float textPosX, Func<float> editBoxWidth)
    {
        ...
        if (!string.Equals(_editEntry, pair.UserData.UID, StringComparison.Ordinal))
        {
            ...
            if (ImGui.IsItemHovered())
            {
                ...
                {
                    _popupTime = DateTime.UtcNow.AddSeconds(_stellarConfigService.Current.ProfileDelay);
                }

                _lastMouseOverUid = id;

                if (_popupTime > DateTime.UtcNow || !_stellarConfigService.Current.ProfilesShow)
                {
    ...
    public (bool isUid, string text) GetPlayerText(Pair pair)
    {
        ...
        if (_stellarConfigService.Current.ShowCharacterNameInsteadOfNotesForVisible && pair.IsVisible && !showUidInsteadOfName)
        {
            playerText = pair.PlayerName;
            textIsUid = false;
            if (_stellarConfigService.Current.PreferNotesOverNamesForVisible)
            {
