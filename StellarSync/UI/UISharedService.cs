...
public partial class UiSharedService : DisposableMediatorSubscriberBase
{
    ...
    private const string _notesEnd = "##STELLAR_SYNC_USER_NOTES_END##";
    private const string _notesStart = "##STELLAR_SYNC_USER_NOTES_START##";
    ...
    public UiSharedService(ILogger<UiSharedService> logger, IpcManager ipcManager, ApiController apiController,
        ...
        StellarConfigService configService, DalamudUtilService dalamudUtil, IDalamudPluginInterface pluginInterface,
        ITextureProvider textureProvider,
        Dalamud.Localization localization,
        ServerConfigurationManager serverManager, TokenProvider tokenProvider, StellarMediator mediator) : base(logger, mediator)
    {
        ...
        {
            e.OnPreBuild(tk => tk.AddDalamudAssetFont(Dalamud.DalamudAsset.NotoSansCjkMedium, new SafeFontConfig
            {
                SizePx = 35,
                FontNo = 0, // Japanese face index in TTC (API 15)
            }));
        });
    ...
    {
        ColorTextWrapped("Note: The storage folder should be somewhere close to root (i.e. C:\\StellarStorage) in a new empty folder. DO NOT point this to your game folder. DO NOT point this to your Penumbra folder.", ImGuiColors.DalamudYellow);
        ...
        ImGui.SameLine();
        using (ImRaii.Disabled(_cacheMonitor.StellarWatcher != null))
        {
            if (IconButton(FontAwesomeIcon.Folder))
            {
                FileDialogManager.OpenFolderDialog("Pick Stellar Sync Storage Folder", (success, path) =>
                {
                    ...
                    {
                        _cacheDirectoryHasOtherFilesThanCache = true;
                        Logger.LogWarning("Found folders in {path} not belonging to Stellar: {dirs}", path, string.Join(", ", dirs));
                    }
                        ...
                        && _cacheDirectoryIsValidPath)
                    {
                        ...
                        _configService.Save();
                        _cacheMonitor.StartStellarWatcher(path);
                        _cacheMonitor.InvokeScan();
                    }
                }, _dalamudUtil.IsWine ? @"Z:\" : @"C:\");
            }
        }
        if (_cacheMonitor.StellarWatcher != null)
        {
        ...
        {
            ColorTextWrapped("Your selected directory has files or directories inside that are not Stellar related. Use an empty directory or a previous Stellar storage directory only.", ImGuiColors.DalamudRed);
        }
        ...
        DrawHelpText("The storage is automatically governed by Stellar. It will clear itself automatically once it reaches the set capacity by removing the oldest unused files. You typically do not need to clear it yourself.");
    }
