...
public class TopTabMenu
{
    ...
    public TopTabMenu(StellarMediator stellarMediator, ApiController apiController, PairManager pairManager, UiSharedService uiSharedService)
    {
    ...
    public string Filter
    {
        ...
        private set
        {
            ...
            {
                _stellarMediator.Publish(new RefreshUiMessage());
            }
    ...
    private void DrawSyncshellMenu(float availableWidth, float spacingX)
    {
            ...
            .Count(g => string.Equals(g.OwnerUID, _apiController.UID, StringComparison.Ordinal)) >= _apiController.ServerInfo.MaxGroupsCreatedByUser))
        {
            ...
            {
                _stellarMediator.Publish(new UiToggleMessage(typeof(CreateSyncshellUI)));
            }
            ImGui.SameLine();
        }
        ...
        using (ImRaii.Disabled(_pairManager.GroupPairs.Select(k => k.Key).Distinct().Count() >= _apiController.ServerInfo.MaxGroupsJoinedByUser))
        {
            ...
            {
                _stellarMediator.Publish(new UiToggleMessage(typeof(JoinSyncshellUI)));
            }
        }
    }
    ...
    {
        var buttonX = (availableWidth - spacingX) / 2f;
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.UserCircle, "Edit Stellar Profile", buttonX))
        {
            _stellarMediator.Publish(new UiToggleMessage(typeof(EditProfileUi)));
        }
        UiSharedService.AttachToolTip("Edit your Stellar Profile");
        ImGui.SameLine();
        if (_uiSharedService.IconTextButton(FontAwesomeIcon.PersonCircleQuestion, "Chara Data Analysis", buttonX))
        {
            _stellarMediator.Publish(new UiToggleMessage(typeof(DataAnalysisUi)));
        }
        ...
        {
            _stellarMediator.Publish(new UiToggleMessage(typeof(CharaDataHubUi)));
        }
    }
