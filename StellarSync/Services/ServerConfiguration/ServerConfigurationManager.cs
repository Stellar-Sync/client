...
public class ServerConfigurationManager
{
    ...
    public ServerConfigurationManager(ILogger<ServerConfigurationManager> logger, ServerConfigService configService,
        ...
        StellarConfigService stellarConfigService, HttpClient httpClient, StellarMediator stellarMediator)
    {
        _logger = logger;
    ...
    {
        if (!_stellarConfigService.Current.AutoPopulateEmptyNotesFromCharaName
    ...
    public async Task<Dictionary<string, string>> GetUIDsWithDiscordToken(string serverUri, string token)
    {
        try
        {
            ...
            var oauthCheckUri = StellarAuth.GetUIDsFullPath(new Uri(baseUri));
    ...
    public async Task<Uri?> CheckDiscordOAuth(string serverUri)
    {
        ...
        {
            var baseUri = serverUri.Replace("wss://", "https://").Replace("ws://", "http://");
            var oauthCheckUri = StellarAuth.GetDiscordOAuthEndpointFullPath(new Uri(baseUri));
    ...
    public async Task<string?> GetDiscordOAuthToken(Uri discordAuthUri, string serverUri, CancellationToken token)
    {
        ...
        {
            var baseUri = serverUri.Replace("wss://", "https://").Replace("ws://", "http://");
            var oauthCheckUri = StellarAuth.GetDiscordOAuthTokenFullPath(new Uri(baseUri), sessionId);
