...
public sealed class CharaDataFileHandler : IDisposable
{
    ...
    private readonly StellarCharaFileDataFactory _stellarCharaFileDataFactory;
            ...
            DalamudUtilService dalamudUtilService, GameObjectHandlerFactory gameObjectHandlerFactory, PlayerDataFactory playerDataFactory)
    {
        ...
        _stellarCharaFileDataFactory = new(fileCacheManager);
    }
    ...
    public Task<(StellarCharaFileHeader loadedCharaFile, long expectedLength)> LoadCharaFileHeader(string filePath)
    {
        try
        {
            ...
            var loadedCharaFile = StellarCharaFileHeader.FromBinaryReader(filePath, reader);

            _logger.LogInformation("Read Stellar Chara File");
    ...
    public Dictionary<string, string> McdfExtractFiles(StellarCharaFileHeader? charaFileHeader, long expectedLength, List<string> extractedFiles)
    {
        ...
        using var reader = new BinaryReader(lz4Stream);
        StellarCharaFileHeader.AdvanceReaderToData(reader);

        long totalRead = 0;
        Dictionary<string, string> gamePathToFilePath = new(StringComparer.Ordinal);
        foreach (var fileData in charaFileHeader.CharaFileData.Files)
        {
            var fileName = Path.Combine(_fileCacheManager.CacheFolder, "stellar_" + _globalFileCounter++ + ".tmp");
    ...
    internal async Task SaveCharaFileAsync(string description, string filePath)
    {
        ...
        try
        {
            ...
            var stellarCharaFileData = _stellarCharaFileDataFactory.Create(description, data);
            StellarCharaFileHeader output = new(StellarCharaFileHeader.CurrentVersion, stellarCharaFileData);
        ...
        {
            _logger.LogError(ex, "Failure Saving Stellar Chara File, deleting output");
