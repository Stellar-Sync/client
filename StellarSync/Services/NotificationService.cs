...
using NotificationType = StellarSync.StellarConfiguration.Models.NotificationType;
...
public class NotificationService : DisposableMediatorSubscriberBase, IHostedService
{
    ...
    private readonly StellarConfigService _configurationService;

    public NotificationService(ILogger<NotificationService> logger, StellarMediator mediator,
        DalamudUtilService dalamudUtilService,
        INotificationManager notificationManager,
        IChatGui chatGui, StellarConfigService configurationService) : base(logger, mediator)
    {
    ...
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Stellar Sync] Error: " + message);
        _chatGui.PrintError(se.BuiltString);
    }

    private void PrintInfoChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Stellar Sync] Info: ").AddItalics(message ?? string.Empty);
        _chatGui.Print(se.BuiltString);
    }

    private void PrintWarnChat(string? message)
    {
        SeStringBuilder se = new SeStringBuilder().AddText("[Stellar Sync] ").AddUiForeground("Warning: " + (message ?? string.Empty), 31).AddUiForegroundOff();
        _chatGui.Print(se.BuiltString);
    }
