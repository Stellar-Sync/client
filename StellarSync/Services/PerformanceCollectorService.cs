...
public sealed class PerformanceCollectorService : IHostedService
{
    ...
    public PerformanceCollectorService(ILogger<PerformanceCollectorService> logger, StellarConfigService stellarConfigService)
    {
        _logger = logger;
        _stellarConfigService = stellarConfigService;
    }

    public T LogPerformance<T>(object sender, StellarInterpolatedStringHandler counterName, Func<T> func, int maxEntries = 10000)
    {
        if (!_stellarConfigService.Current.LogPerformance) return func.Invoke();
    ...
    public void LogPerformance(object sender, StellarInterpolatedStringHandler counterName, Action act, int maxEntries = 10000)
    {
        if (!_stellarConfigService.Current.LogPerformance) { act.Invoke(); return; }
    ...
    {
        if (!_stellarConfigService.Current.LogPerformance)
        {
